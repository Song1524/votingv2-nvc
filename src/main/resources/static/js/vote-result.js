...
async function loadResults() {
    try {
        // 1. DB 기준 투표 결과 가져오기
        const voteRes = await fetch(`https://votingv2-nvc-production.up.railway.app/api/votes/${voteId}`, {
        ...
        for (let item of vote.items) {
            const countRes = await fetch(
                `https://votingv2-nvc-production.up.railway.app/api/votes/${voteId}/items/${item.itemId}/count`,
                { headers: { Authorization: "Bearer " + token } }
            );
      ...
      `;
        ...
        const blockchainRes = await fetch(
            `https://votingv2-nvc-production.up.railway.app/api/votes/${voteId}/results/blockchain`,
            { headers: { Authorization: "Bearer " + token } }
        );
